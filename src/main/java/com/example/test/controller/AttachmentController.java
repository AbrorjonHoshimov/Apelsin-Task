...
import com.example.test.repository.AttachmentContentRepository;
...
//    }
    ...
    public void getSystem(@PathVariable Integer id, HttpServletResponse response) throws IOException {
        ...
        response.setContentType(attachment.getType());
        response.setHeader("Content-Disposition", attachment.getOriginalName());

        //systemdan topishi
        FileInputStream fileInputStream = new FileInputStream(new File(uploadDirectory + "/" + attachment.getOriginalName()));
        //copy qilib response
